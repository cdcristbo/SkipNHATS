...
for (file in file_list) {
  ...
  HCAll <- data %>% 
    select(spid, ends_with("dresid"), starts_with("cg"))
  ...
  HC <- HCAll %>% 
    select(spid, max_value_column, starts_with("cg")) 
  
  # Get the names of the columns starting with "hc"
  hc_columns <- grep("^cg", names(HC), value = TRUE)
...
result_df <- result_df %>%  # Assign the modified dataframe back to result_df
  ...
  mutate(round = as.integer(gsub("cg(\\d+).*", "\\1", variable))) %>%  # Extract the numeric part from the 'variable' column and assign it to the new column 'round'
  mutate(label = sub("cg\\d+", "", variable)) %>%  # Remove the 'hc' prefix and the numeric part from the 'variable' column and assign it to the new column 'label'
